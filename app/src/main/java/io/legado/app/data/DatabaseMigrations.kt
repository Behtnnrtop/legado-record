...
object DatabaseMigrations {
    ...
    val migration_89_90 = object : Migration(89, 90) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL(
                """
                    CREATE TABLE IF NOT EXISTS `bookReadDayRecords` (
                        ...
                        PRIMARY KEY(`deviceId`, `bookUrl`, `date`)
                    )
                """.trimIndent()
            )
            db.execSQL(
                """
                    CREATE INDEX IF NOT EXISTS `index_bookReadDayRecords_bookUrl`
                    ON `bookReadDayRecords` (`bookUrl`)
                """.trimIndent()
            )
            db.execSQL(
                """
                    CREATE INDEX IF NOT EXISTS `index_bookReadDayRecords_bookUrl_date`
                    ON `bookReadDayRecords` (`bookUrl`, `date`)
                """.trimIndent()
            )
        }
    }
    ...
    val migration_90_91 = object : Migration(90, 91) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL(
                """
                    CREATE TABLE IF NOT EXISTS `bookReadSessions` (
            ...
            db.execSQL(
                """
                    CREATE INDEX IF NOT EXISTS `index_bookReadSessions_bookUrl`
                    ON `bookReadSessions` (`bookUrl`)
                """.trimIndent()
            )
            db.execSQL(
                """
                    CREATE INDEX IF NOT EXISTS `index_bookReadSessions_bookUrl_date`
                    ON `bookReadSessions` (`bookUrl`, `date`)
                """.trimIndent()
            )
            db.execSQL(
                """
                    CREATE INDEX IF NOT EXISTS `index_bookReadSessions_deviceId_bookUrl_type_date`
                    ON `bookReadSessions` (`deviceId`, `bookUrl`, `type`, `date`)
                """.trimIndent()
            )
            db.execSQL(
                """
                    CREATE UNIQUE INDEX IF NOT EXISTS `index_bookReadSessions_deviceId_bookUrl_type_startTime_endTime`
                    ON `bookReadSessions` (`deviceId`, `bookUrl`, `type`, `startTime`, `endTime`)
                """.trimIndent()
            )

            db.execSQL("ALTER TABLE `bookReadDayRecords` RENAME TO `bookReadDayRecords_old`")
            db.execSQL(
                """
                    CREATE TABLE IF NOT EXISTS `bookReadDayRecords` (
                        `bookUrl` TEXT NOT NULL,
            ...
            db.execSQL(
                """
                    INSERT INTO `bookReadDayRecords` (
                        `bookUrl`, `bookName`, `bookAuthor`, `date`,
                    ...
                    SELECT
                        ...
                        coalesce(min(nullif(`startTime`, 0)), 0) AS `startTime`,
                        max(`lastRead`) AS `lastRead`
                    FROM `bookReadDayRecords_old`
                    GROUP BY `bookUrl`, `date`
                """.trimIndent()
            )
            db.execSQL("DROP TABLE `bookReadDayRecords_old`")
            db.execSQL(
                """
                    CREATE INDEX IF NOT EXISTS `index_bookReadDayRecords_bookUrl`
                    ON `bookReadDayRecords` (`bookUrl`)
    ...
    val migration_91_92 = object : Migration(91, 92) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL(
                """
                    CREATE INDEX IF NOT EXISTS `index_bookReadDayRecords_date`
                    ON `bookReadDayRecords` (`date`)
                """.trimIndent()
            )
        }
    }
    ...
    val migration_92_93 = object : Migration(92, 93) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL(
                """
                    CREATE TABLE IF NOT EXISTS `readRecord_new` (
                        ...
                        `readTime` INTEGER NOT NULL DEFAULT 0,
                        `lastRead` INTEGER NOT NULL DEFAULT 0,
                        PRIMARY KEY(`deviceId`, `recordKey`)
                    )
                """.trimIndent()
            )
            db.execSQL(
                """
                    INSERT INTO `readRecord_new` (
                        `deviceId`, `recordKey`, `bookUrl`, `bookName`, `bookAuthor`,
                        `readTime`, `lastRead`
                    )
                    SELECT
                        `deviceId`,
            ...
            db.execSQL(
                """
                    CREATE TABLE IF NOT EXISTS `readRecordLegacyMappings` (
                        `deviceId` TEXT NOT NULL,
                        `bookName` TEXT NOT NULL,
                        `bookUrl` TEXT NOT NULL,
            ...
            db.execSQL(
                """
                    CREATE INDEX IF NOT EXISTS `index_readRecordLegacyMappings_bookUrl`
                    ON `readRecordLegacyMappings` (`bookUrl`)
                """.trimIndent()
            )
        }
    }
    ...
    val migration_93_94 = object : Migration(93, 94) {
        override fun migrate(db: SupportSQLiteDatabase) {
            ...
            db.execSQL("ALTER TABLE books ADD COLUMN ratingUpdateTime INTEGER NOT NULL DEFAULT 0")
        }
    }
