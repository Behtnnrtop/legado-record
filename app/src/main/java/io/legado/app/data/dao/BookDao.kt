...
interface BookDao {
    ...
    fun flowShelfByRating(rating: Float): Flow<List<Book>> {
        return flowByRating(rating).map { list ->
            list.filterNot { it.isNotShelf }
        }
    }
    ...
    @Query("SELECT * FROM books where type & ${BookType.text} > 0 ORDER BY durChapterTime DESC limit :limit")
    fun getRecentReadBooks(limit: Int): List<Book>
