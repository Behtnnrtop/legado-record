    ...
    private suspend fun restore(path: String) {
        ...
        fileToListT<ReadRecord>(path, "readRecord.json")?.let {
            it.forEach { readRecord ->
                if (readRecord.recordKey.isBlank()) {
                    readRecord.recordKey = if (readRecord.bookUrl.isNotBlank()) {
                        ReadRecord.urlRecordKey(readRecord.bookUrl)
                    } else {
                        ReadRecord.legacyRecordKey(readRecord.bookName)
                    }
                }
                if (readRecord.bookUrl.isBlank() && readRecord.recordKey.startsWith("url:")) {
                    readRecord.bookUrl = readRecord.recordKey.removePrefix("url:")
                }
                //判断是不是本机记录
                if (readRecord.deviceId != androidId) {
                    ...
                    val time = appDb.readRecordDao
                        .getReadTimeByKey(readRecord.recordKey)
        ...
        fileToListT<BookReadSession>(path, "bookReadSessions.json")?.let {
            BookReadDayRecordModel.restoreSessions(it)
        }
