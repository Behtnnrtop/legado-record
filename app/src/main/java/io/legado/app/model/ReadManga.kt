...
object ReadManga : CoroutineScope by MainScope() {
    ...
    fun resetData(book: Book) {
        ReadManga.book = book
        readRecord.deviceId = io.legado.app.constant.AppConst.androidId
        ...
        readRecord.readTime = appDb.readRecordDao
            .getByKey(readRecord.deviceId, readRecord.recordKey)?.readTime ?: 0
    ...
    fun upReadTime() {
        ...
        readStartTime = now
        executor.execute {
            BookReadDayRecordModel.commitReadDuration(
                book = book,
                readRecord = readRecord,
                startTime = startTime,
                endTime = now,
                type = BookReadDayRecordModel.TYPE_MANGA
            )
        }
    }
