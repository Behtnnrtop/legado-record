...
import io.legado.app.model.BookReadRecordMigrationModel
...
class BookInfoViewModel(application: Application) : BaseViewModel(application) {
    ...
    val readStatsData = MutableLiveData<BookReadStats>()
    ...
    private fun upBook(book: Book) {
        execute {
            ...
            readStatsData.postValue(getBookReadStats(book))
            upCoverByRule(book)
        ...
        if (book.isLocal) {
            ...
            WebBook.getBookInfo(scope, bookSource, book, canReName = canReName)
                .onSuccess(IO) {
                    ...
                    readStatsData.postValue(getBookReadStats(it))
                    if (inBookshelf) {
                        it.save()
                    }
                    if (it.isWebFile) {
                ...
                .onSuccess(IO) {
                    if (inBookshelf) {
                        ...
                        if (oldBook.bookUrl != book.bookUrl) {
                            BookReadRecordMigrationModel.migrateOnBookSourceChanged(oldBook, book)
                        }
    ...
    fun updateRating(rating: Float) {
        ...
        execute {
            ...
            if (appDb.bookDao.has(book.bookUrl)) {
                appDb.bookDao.updateRating(book.bookUrl, normalized, updateTime)
            } else {
                book.save()
            }
        }.onSuccess {
            if (normalized > 0f) {
                context.toastOnUi(context.getString(R.string.book_rating_updated, normalized))
            } else {
                context.toastOnUi(R.string.book_rating_cleared)
            }
        }
    }
    ...
    private fun loadReadStats(book: Book) {
        execute {
            readStatsData.postValue(getBookReadStats(book))
        }
    }

    fun getBookReadStats(book: Book): BookReadStats {
        val dayRecords = appDb.bookReadDayRecordDao.getByBook(book.bookUrl)
        val recordStats = ReadRecordTotalModel.getBookStats(book)
        val totalReadTime = recordStats.totalReadTime
        if (dayRecords.isNotEmpty()) {
            return BookReadStats(
                totalReadTime = totalReadTime,
                readDayCount = dayRecords.count { it.readTime > 0L },
                firstReadTime = dayRecords
                    .map { it.startTime }
                    .filter { it > 0L }
                    .minOrNull(),
                lastReadTime = dayRecords
                    .map { it.lastRead }
                    .filter { it > 0L }
                    .maxOrNull(),
        ...
        return BookReadStats(
            totalReadTime = totalReadTime,
            lastReadTime = recordStats.lastRead,
            hasDayRecords = false
        )
    }
